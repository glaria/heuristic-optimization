...
results = pd.DataFrame(columns = ['instanceName', 'alpha', 'Reward', 'Route Cost', 'Computational Time'])
#instanceName = 'p5.3.c' # name of the instance
# txt file with the TOP instance data
instance_list = ['p5.3.q', 'p3.3.m', 'p1.3.k', 'p5.3.c']    
for alpha_index in range(11):
    ...
    for  instanceName in instance_list:
       ...
       # row_df = pd.DataFrame([pd.Series([instanceName, alpha, round(sol.demand,2), round(sol.cost,2),round(endTime-startTime,2) ] )])

        #results = pd.concat([row_df, results], ignore_index=True)   
        results.loc[-1] = [instanceName, alpha, round(sol.demand,2), round(sol.cost,2),round(endTime-startTime,2) ]
        results.index = results.index + 1
        results = results.sort_index()
results = results.sort_values(["instanceName", "alpha"])
print(results)
results.to_csv('pjs_data_analysis.csv')
