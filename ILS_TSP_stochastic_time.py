...
import random, time
import numpy as np
from Stochastic_berlin52 import euclideanDistance, generateStochasticDataset
import pandas as pd
import warnings
...
#calculates travel time between 2 nodes assuming a stochastic approach
def stochasticTime(xNode, yNode):
    mean = inputsTSP_stochastic[str(xNode)][str(yNode)][0]
    variance = inputsTSP_stochastic[str(xNode)][str(yNode)][1] #just in case its needed
    stochastic_time = np.random.exponential(mean)
    return stochastic_time

def stochasticTourCost(perm):
    # Here tour cost refers to the sum of the stochastic times between consecutive points starting from first element
    totalTime =0.0
    size = len(perm)
    for index in range(size):
        # select the consecutive point for calculating the segment length
        ...
        totalTime +=  stochasticTime(perm[index], point2)
    
    return totalTime   

''' Monte Carlo simulation'''

def MCsimulation(perm, maxIter):
    simulation_values = np.array([])
    for i in range(maxIter):
        simulation_values = np.insert(simulation_values, 0,stochasticTourCost(perm))
    
    return simulation_values.mean()

''' ALGORITHM FRAMEWORK'''

algorithmName = "ILS with stochastic times"
...
inputsTSP_stochastic = generateStochasticDataset(berlin52)
...
for i in range(200):
    ...
    #bestStochasticCost = stochasticTourCost(bestSol)
    #bestStochasticSol = bestSol
    #print("Average cost simulated: ", MCsimulation(bestSol, maxMCsim))
    ...
    while maxIterations_mod >0:
        ...
        if newCost < bestCost:
            stochastic_cost = MCsimulation(newSol, maxMCsim)
            if stochastic_cost < bestCost:
                bestSol = newSol
                bestCost = newCost
            #print("Average cost simulated: ", MCsimulation(bestSol, maxMCsim))
...
results.to_csv('results_ILS_stochastic_times.csv')
